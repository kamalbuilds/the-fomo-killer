...
export class UtilityAgent extends BaseAgent {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.logger.warn('Continuing without CDP wallet functionality');
      ...
      // Create a basic LLM model if not already created
      if (!this.llmModel) {
        this.llmModel = new ChatOpenAI({
      ...
      // Create React Agent with basic tools only
      this.reactAgent = createReactAgent({
        llm: this.llmModel,
        tools: this.tools,
        checkpointSaver: this.memory,
      ...
      await super.initialize();
      this.logger.info('UtilityAgent initialized without CDP wallet (authentication failed)');
    }
  }
