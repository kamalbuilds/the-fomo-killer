...
export class SentimentAgent extends BaseAgent {
  ...
  private x402PaymentEnabled: boolean = true;
  ...
  async initialize(): Promise<void> {
    try {
      // Initialize DeFi Analytics agent for cross-agent collaboration
...
${message ? `Message: ${message}` : ''}`;
      ...
      new DynamicStructuredTool({
        name: 'get_defi_sentiment_analysis',
        description: 'Get DeFi-specific sentiment analysis combining market data with social sentiment',
        schema: z.object({
          tokens: z.array(z.string()).optional().default(['ETH', 'USDC']),
          includeAnalytics: z.boolean().optional().default(true),
        }),
        func: async ({ tokens, includeAnalytics }) => {
          try {
            // Get sentiment data
            ...
            // If DeFi analytics requested and available
            let analyticsData = null;
            if (includeAnalytics && this.defiAnalytics) {
              // Create a mock message for DeFi analytics
              const mockMessage: DecodedMessage = {
                content: `Get analytics for ${tokens.join(', ')}`,
                senderInboxId: 'sentiment-agent',
                sent: new Date(),
                contentType: { typeId: 'text' }
              } as DecodedMessage;
              
              const analyticsResponse = await this.defiAnalytics.handleMessage(mockMessage, context);
              analyticsData = analyticsResponse.message;
            }

            return `📊 DeFi Sentiment Analysis:
${sentimentData.map(d => `${d.token}: ${d.sentiment.score}% positive`).join('\n')}

${analyticsData ? `\n📈 Market Analytics:\n${analyticsData}` : ''}

💡 Combined Analysis: ${this.generateCombinedInsight(sentimentData)}`;
          } catch (error) {
            this.logger.error('Error in DeFi sentiment analysis', { error });
            return `Error analyzing DeFi sentiment: ${error instanceof Error ? error.message : 'Unknown error'}`;
          }
        },
      }),
...
- keyPoints: array of strings`;
  ...
  protected async handleMessage(message: DecodedMessage, context: AgentContext): Promise<AgentResponse> {
    try {
      ...
      // Check for x402 payment for premium features
      const isPremium = await this.checkX402Payment(context);
      ...
      const finalMessage = response.trim() || 'I can help you with crypto news, social content, and community engagement. What would you like to explore?';
      
      return {
        message: isPremium ? finalMessage : finalMessage + this.createX402PaymentPrompt(),
        metadata: { 
...
You provide real social intelligence and blockchain-powered community engagement.`;
  ...
  private generateCombinedInsight(sentimentData: Array<{ token: string; sentiment: { score: number; source: string } }>): string {
    const avgScore = sentimentData.reduce((sum, d) => sum + d.sentiment.score, 0) / sentimentData.length;
    
    if (avgScore > 70) {
      return 'Strong bullish sentiment across tokens. Market confidence is high.';
    } else if (avgScore > 50) {
      return 'Moderate positive sentiment. Market showing cautious optimism.';
    } else if (avgScore > 30) {
      return 'Mixed sentiment detected. Market uncertainty prevails.';
    } else {
      return 'Bearish sentiment dominant. Risk management recommended.';
    }
  }

  // x402 Payment Integration for Premium Features
  private async checkX402Payment(context: AgentContext): Promise<boolean> {
    if (!this.x402PaymentEnabled) return true;
    
    // Check for x402 payment in agent memory
    const paymentVerified = context.agentMemory?.['x402-payment-verified'] as boolean;
    return !!paymentVerified;
  }

  private createX402PaymentPrompt(): string {
    return `\n\n💎 Premium features available with x402 micro-payment (0.0005 ETH):
• Real-time sentiment analysis
• DeFi market integration
